...
selector_t*
selector_list_init( SDL_Renderer* renderer, config_t* game_config )
{
        config = game_config;
        selector_index = 0;
        ...
        int O_option_x = 50;
        int O_option_y = 600;
        label_create( renderer, "O", O_option_x+7, O_option_y+7, 60 );
        o_marker_toggle = selector_init( renderer,
                        O_option_x,
                        O_option_y,
                        50,
                        50,
        ...
        int X_option_x = 120;
        int X_option_y = 600;
        label_create( renderer, "X", X_option_x + 7, X_option_y + 7, 60 );
        x_marker_toggle = selector_init( renderer,
                        X_option_x,
                        X_option_y,
                        50,
                        50,
        ...
        label_create( renderer, "SMART", SMART_option_x + 10, SMART_option_y + 5, 35 );
        smart_style_toggle = selector_init( renderer,
                        SMART_option_x, 
                        SMART_option_y,
                        130,
                        40,
        ...
        label_create( renderer, "RAND", RAND_option_x + 15, RAND_option_y + 5, 40 );
        rand_style_toggle = selector_init( renderer,
                        RAND_option_x,
                        RAND_option_y,
                        130,
                        40,
...
selector_t*
selector_init( SDL_Renderer* renderer,
        int x,
        int y,
        int w,
        int h,
        ...
        int (*unpressFunc)() )
{
        ...
        if ( text != NULL )
        {
                label_create( renderer, text, x+7, y+7, 40 );
        }
...
void
draw_selector( SDL_Renderer* renderer, selector_t* selector )
{
        if ( selector->is_highlighted )
        {
                set_color( renderer, *selector->highlight_color );
        }
        else if ( selector->is_selected )
        {
                set_color( renderer, *selector->selected_color );
        }
        else
        {
                set_color( renderer, *selector->color );
        }
        ...
        if ( selector->is_pressed )
        {
                ...
                {
                        set_color( renderer, *selector->pressed_color );
                }
        }

        SDL_RenderFillRect( renderer, &selector->box );
}
...
void
draw_selector_list( SDL_Renderer* renderer )
{
        for ( int i = 0; i < selector_index; i++ )
        {
                draw_selector( renderer, selector_list[ i ] );
        }
}
